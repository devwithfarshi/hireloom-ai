...
export function JobList({
  ...
  const [selectedJobAnalysis, setSelectedJobAnalysis] = useState<{
    jobTitle: string;
    matchAnalysis: AiJob["matchAnalysis"];
    relevanceScore?: number;
  } | null>(null);
    ...
    <>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {jobs.map((job) => {
          ...
          const hasAiData =
            isAiMode &&
            (aiJob.relevanceScore !== undefined || aiJob.matchAnalysis);
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    {hasAiData && aiJob.relevanceScore !== undefined && (
                      <div
                        className={`flex items-center gap-1 px-2 py-1 rounded-full border text-xs font-medium ${getScoreColor(aiJob.relevanceScore)}`}
                      >
                  ...
                  {hasAiData && aiJob.matchAnalysis && (
                    <div className="mt-4 p-3 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-200/50">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <div
                          className={`ml-auto flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getScoreColor(aiJob.matchAnalysis.score)}`}
                        >
                        ...
                        <div className="mb-2">
                          ...
                          {aiJob.matchAnalysis.strengths.length > 1 && (
                            <span className="text-xs text-green-600">
                              +{aiJob.matchAnalysis.strengths.length - 1} more
                            </span>
                          )}
                        </div>
                      )}
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        className="w-full mt-2 text-xs bg-white/80 hover:bg-white border-blue-300 text-blue-700 hover:text-blue-800"
                        onClick={() =>
                          setSelectedJobAnalysis({
                            jobTitle: job.title,
                            matchAnalysis: aiJob.matchAnalysis!,
                            relevanceScore: aiJob.relevanceScore,
                          })
                        }
                      >
      ...
      {selectedJobAnalysis && (
        <AiAnalysisModal
          isOpen={!!selectedJobAnalysis}
          onClose={() => setSelectedJobAnalysis(null)}
          jobTitle={selectedJobAnalysis.jobTitle}
          matchAnalysis={selectedJobAnalysis.matchAnalysis!}
          relevanceScore={selectedJobAnalysis.relevanceScore}
        />
      )}
    </>
  );
